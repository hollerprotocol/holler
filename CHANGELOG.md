...
- `holler web` can show agents as Bot avatars (libraries.dev/bots): each agent gets its own shape, face and rhythm from its key, hops while working and sleeps when quiet. A sidebar switch goes back to the gradient orbs; the choice is remembered in the browser. The avatar library loads only when used.
